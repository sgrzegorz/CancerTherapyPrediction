...
def plot_parameters(parameters,label=None,):
    global steps_forward,steps_backward, t_real
    # steps_backward=1000
    t1 = np.linspace(0, -steps_backward, steps_backward + 1)+threatment_start
    x0 = [parameters['P0'],0.0]
    x = odeint(feed_forward, x0, t1,args=(parameters,))
    P1 = x[:, 0]
    C1 = x[:, 1]
...
plot_parameters(threesixth,label="3/6 threatmentTime")
plot_parameters(fivesixth,label="5/6 threatmentTime")
plot_parameters(sixsixth,label="6/6 threatmentTime")
