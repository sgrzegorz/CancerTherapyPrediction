...
Bounds = [[0.1*m, 10*m] for m in idealX]
...
def DirectOperator( X ):
    lambda_p, K, gamma_p, eta, KDE = numpy.ravel(X).tolist()
    #
...
PlotOneSerie(Observations,t,plt1,"Observations 3DVAR2",color='black')
PlotOneSerie(DirectOperator(idealX),t,plt1,"Simulation for Xtab1pcv 3DVAR2",)
PlotOneSerie(DirectOperator(Xa[-1]),t,plt1,"Simulation for Xa 3DVAR2")
plt.show()
